...
export const SimulateTransaction = ({ network, contractABI, inspectFunction }) => {
  ...
  const { REACT_APP_TENDERLY_USER, REACT_APP_TENDERLY_PROJECT, REACT_APP_TENDERLY_ACCESS_KEY } = process.env;
  ...
  useEffect(() => {
    if (simulationReady) {
      const inputTypes = Object.values(functionInputs).map((param) => param.type);
  
      const inputValues = Object.values(functionInputs).map((param) => param.value);
      ...
      const encodedParams = ethers.utils.defaultAbiCoder.encode(inputTypes, inputValues);
  ...
  return (
    <div style={{ margin: '0 auto' }}>
      ...
      {simulationValid ? (
        ...
        <Button 
        isDisabled={!simulationReady}
         onClick={transaction}>Simulation Transaction</Button>
        </Box>
          
      ) : (
        <p>
          This transaction cannot be simulated. The function is either
          internal or private.
        </p>
      )}
    </div>
