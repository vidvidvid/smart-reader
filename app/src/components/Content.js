...
export const Content = ({ address, fetching, setFetching }) => {
    ...
    useEffect(() => {
        if (hasCopied) {
            toast({
                ...
                description: `The contract address for ${tokenData.name} has been copied to your clipboard.`,
                status: 'success',
                duration: 5000,
                isClosable: true,
            })
    ...
    useEffect(() => {
        if (userAddress && address && address.length > 0) {
        ...
        async (code, type, arrayId = '') => {
            ...
            if (!fileExplanationSuccess) {
                ...
                if (type === explanation.contract) {
                    ...
                    requiredField = 'dependency_explanations';
                    content = `Give me a simple explanation of the following solidity file or dependency: ${code}. Please use markdown formatting in all responses`;
                    setIsLoadingDependency(true);
                } else {
                    requiredField = 'function_explanations';
                    content = `Give me a simple explanation of the following solidity code: ${code}. Please use markdown formatting in all responses`;
                    setIsLoadingFunction(true);
                }
            ...
            if (lineNode.nodeName === 'SPAN') {
                ...
                childSpans.forEach((span) => {
                    ...
                    if (span.innerText.includes('function')) {
                        ...
                        // apend the button to the first line of the function
    ...
    return (
        ...
        >
          ...
          zIndex={{ base: 'unset', lg: -1 }}
                >
                    <Flex
                        ...
                        overflow="hidden"
                    >
                        Connect your wallet to start. <ArrowUpIcon />
                    </Flex>
                </Box>
            ) : undefined}
            ...
            {!userAddress ? (
                ...
                <>
                    <Stack w="full" justifyContent="stretch">
          ...
          {address !== '' && <Comments chainId={chain?.id} contractAddress={address} />}
            ...
            <Modal isOpen={isOpenAnnotation} onClose={onCloseAnnotation} size="7xl">
                <ModalOverlay />
                                            ...
                                            {!isLoadingFunction && (
                                                <Flex direction="column" gap={3}>
                                                    ...
                                                    <Flex
                                                        ...
                                                        <Viewer>{functionExplanation}</Viewer>
                                                    </Flex>
                                                </Flex>
                                            )}
                                        </Box>
